...
# Aggregate data for each type of accidents
...
create_heatmap_plot <- function(data, title_text) {
  ggplot(data, aes(x = TimeOfDay, y = Weekday, fill = Accidents)) +
    geom_tile(color = "white") +
    scale_fill_gradient(low = "white", high = "red") +
    labs(title = title_text, x = "Time of Day", y = "Weekday (reversed order)", fill = "Number of Accidents") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability
}

# Create heatmaps for each accident type
p1 <- create_heatmap_plot(agg_injured_df, "with Injuries")
p2 <- create_heatmap_plot(agg_deadly_df, "with Deaths")
p3 <- create_heatmap_plot(agg_no_injury_or_death_df, "without Injuries or Deaths")

# Arrange the three plots vertically and add a main title
grid.arrange(
  p1, p2, p3,
  nrow = 3,
  top = textGrob("Accidents per Weekday by Time of Day", gp = gpar(fontsize = 20, font = 2)),
  heights = c(1, 1, 1) * 2,
