...
## Files
We count with 2 datasets, one containing the vehicle crashes and the second one about the weather of the location.
...
## Analysis
The following subsections will illustrate the insights gained by generating plots of the given datasets.
...
At the same time, the majority of accidents occur in no rain conditions, which could be due simply to the fact that most of the time there is no rain in New York. Hence, without the total amount of vehicles on the road, it is difficult to draw conclusions from this data alone.
...
```
The shown plots above shall be used as another step into a deeper analysis of when accidents are happening in New York. Therefore, the three spider-radars are all sharing some commonalities. Accidents seem to occur closer to the Weekend than at the beginning of the week, no matter whether it was a small crash without injuries or fatal crashes leading to deaths. With this information, the team decided to dig one layer deeper to find out, on what time, what type of accidents happen mostly and gain some insightful information with that.
...
As mentioned in previous plot, with the knowledge acquired, the team decided to focus now on the Time of day, where accidents occurred in 2021. So the given plot clearly indicates that during Rush Hour (5pm to 8pm) most accidents occur. This may be simply due to the fact, that this is the time, when most people are actually on the road as either pedestrian, biker, or car-driver.
...
With below shiny application, users are able to select a date range (default takes the first and last date of the loaded dataset), the weekday(s) specify whether only accidents with Death(s) and/or injured people should be included it in.
...
![Shiny Application](resources\shiny.pdf)
