...
```{r Temporal Analysis, cache=TRUE}
# 1. Temporal Analysis
## Accidents by Hour
ggplot(accidents, aes(x = Hour, y = ..density..)) +
  geom_density(fill = "skyblue", alpha = 0.7) +
  coord_polar(start = 0) +  # Start the polar plot at 0
  labs(title = "Distribution of Accidents by Hour", x = "", y = "") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(0, 24, by = 1), limits = c(0, 24))  # Set breaks from 0 to 24 and limit the x-axis
...
### Spatial Analysis
Here we will analyze the distribution of accidents by borough and visualize the accident locations on a map.
...
# 9. time of the day (morning, night, evening) with the highest number of accidents
accidents %>%
...
To solve this issue, we can increase the maximum request size for Shiny apps by setting the `options(shiny.maxRequestSize)` option to a higher value. In this case, we set it to 300MB, which should be enough for our dataset.
